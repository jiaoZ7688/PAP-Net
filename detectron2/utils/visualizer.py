...
class Visualizer:
    ...
    def draw_mask_only(self, masks, boxes, colors, name='None'):
        ...
        self.output.save('../data/outtest/{}.png'.format(name))

    def draw_masks(self, masks, name='None', colors=None):
        masks = np.asarray(masks)
        masks = [GenericMask(x, self.output.height, self.output.width) for x in masks]
    ...
    def my_draw_instance_predictions(self, predictions, colors=None):
    ...
    def draw_instance_predictions(self, predictions):
        ...
        if predictions.has("pred_masks"):
            ...
            # masks = [GenericMask(x, self.output.height, self.output.width) for x in masks][:2]
            masks = [GenericMask(x, self.output.height, self.output.width) for x in masks]
    ...
    def overlay_instances(
        ...
        *,
        ...
        if assigned_colors is None:
            ...
            assigned_colors = random_colors(num_instances, maximum=1, rgb=True)
        if num_instances == 0:
            return self.output
    ...
    def draw_polygon(self, segment, color, edge_color=None, alpha=0.5):
        ...
        """
        color = self._change_color_brightness(color, brightness_factor=0.4)
